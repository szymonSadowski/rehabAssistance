    ...
    def draw_humans(npimg, humans, imgcopy=False):
        ...
        for human in humans:
            ...
            for i in range(common.CocoPart.Background.value):
                ...
                centers[i] = center
                # dodanie do listy x,y punktow widocznych na klatce dodatkowowo załączamy dictionary z
                # widocznymi punktami kluczowymi
                pointsx.append(center[0])
                pointsy.append(center[1])
                part = human.body_parts.keys()

                #odpalamy w osobnym wątku zapisywanie do jsona
                if(len(pointsx)>=len(part)):
                      threading.Thread(target=fileEdition.save_points, args=(pointsx,pointsy,part, )).start()
                      pointsx = []
                      pointsy = []
